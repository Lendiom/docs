...
- A modern phone, tablet, or computer with an up-to-date OS and browser (recent iOS/Android, Windows, macOS; current Chrome, Edge, Safari, Firefox).
- A device authenticator (built-in biometrics / device PIN) or a physical security key (USB/Lightning/NFC/Bluetooth).

**Step-by-step**

1. Open **Account Settings > Security**.
2. Choose **Add passkey**
3. Give the passkey a name (e.g., “Office iPhone”) and save.
4. Pick the device you want to use (this device or an external security key).
...
- **Strong, per-site cryptography:** each site gets its own key pair — a stolen key from one site can’t be used at another.
- **Stored securely on your device or in an encrypted cloud keychain:** private keys never leave your device unencrypted.
...
- **Keep a recovery option active** (verified email, account recovery phone, or a backup authenticator) so you can regain access if you lose every registered device.
...
- If adding a passkey fails, try a different browser or register a hardware security key (YubiKey, etc.) as an alternative.
